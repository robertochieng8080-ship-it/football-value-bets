  ...
  return(
    <main className="max-w-6xl mx-auto p-4 md:p-6">
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto">
        ...
        <select value={fEdge} onChange={e=>setFEdge(e.target.value)} className="ml-auto glass rounded-full px-4 py-2 text-xs"><option value="5">Edge 5%+</option><option value="0">All Bets</option></select>
      </div>

      <div className="grid md:grid-cols-2 gap-4">
        {filtered.map(b=><div key={b.fixture_id} className="glass rounded-[20px] p-5 hover:border-zinc-700 transition">
          <div className="flex justify-between text-[11px]"><span className="text-zinc-500 uppercase tracking-wide">{b.league_name} • {b.kickoff_time || 'EAT'}</span><span className="bg-green-500/15 text-green-400 border border-green-500/20 px-2.5 py-0.5 rounded-full font-bold">+{b.edge_percent}% EDGE</span></div>
          <p className="font-bold text-[18px] mt-3 tracking-tight">{b.home_team} vs {b.away_team}</p>
          <div className="flex gap-2 mt-3"><span className="bg-white text-black text-[11px] font-black px-3 py-1 rounded-full">{b.market}</span><span className="glass text-[11px] px-3 py-1 rounded-full text-zinc-400">Exp {b.expected_score}</span><span className="glass text-[11px] px-3 py-1 rounded-full text-green-300 border border-green-500/20">{b.kickoff_time}</span></div>
