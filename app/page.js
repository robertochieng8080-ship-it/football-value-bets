...
export default function Home(){
  ...
  useEffect(()=>{ supabase.from('predictions_today').select('*').order('edge_percent',{ascending:false}).then(r=>setData(r.data||[])) },[])
  ...
  return(
    <main className="max-w-6xl mx-auto p-4 md:p-6">
      ...
      <div className="glass-green rounded-[24px] p-6 mb-6 flex justify-between items-center">
        <div><p className="text-xs tracking-widest text-green-400/70 uppercase">Live Value Pool</p><p className="text-3xl font-black mt-1">{filtered.length} BETS</p><p className="text-sm text-zinc-400 mt-1">Quality {'>'} Quantity. Edge {'<'}5% hidden.</p></div>
      ...
      <div className="flex gap-2 mb-6">
        {['All','Over2.5','BTTS_Yes','1_Over2.5'].map(m=><button key={m} onClick={()=>setFMarket(m)} className={`px-4 py-2 rounded-full text-xs border ${fMarket===m?'bg-white text-black border-white font-bold':'glass text-zinc-400 border-zinc-800'}`}>{m}</button>)}
        <select value={fEdge} onChange={e=>setFEdge(e.target.value)} className="ml-auto glass rounded-full px-4 py-2 text-xs"><option value="5">Edge 5%+ Quality</option><option value="10">Edge 10%+ High</option></select>
      </div>
      ...
      {filtered.length===0? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {filtered.map(b=><div key={b.id} className="glass rounded-[20px] p-5 hover:border-zinc-700 transition">
            <div className="flex justify-between text-[11px]"><span className="text-zinc-500 uppercase tracking-wide">{b.league_name}</span><span className="bg-green-500/15 text-green-400 border border-green-500/20 px-2.5 py-0.5 rounded-full font-bold">+{b.edge_percent}% EDGE</span></div>
            <p className="font-bold text-[18px] mt-3 tracking-tight">{b.home_team} vs {b.away_team}</p>
            <div className="flex gap-2 mt-3"><span className="bg-white text-black text-[11px] font-black px-3 py-1 rounded-full">{b.market}</span><span className="glass text-[11px] px-3 py-1 rounded-full text-zinc-400">Exp {b.expected_score}</span></div>
